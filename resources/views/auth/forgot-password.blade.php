<x-guest-layout>
    @section('title', 'AuraCampus | Recovery Keys')
    ...
    <div class="min-h-screen w-screen flex flex-col items-center justify-center p-6 tech-canvas relative overflow-hidden select-none bg-[#fafbfc]">
        ...
        <div class="w-full max-w-[420px] p-8 rounded-2xl bg-white border border-slate-200/60 shadow-[0_1px_3px_rgba(0,0,0,0.01),0_24px_48px_-16px_rgba(15,23,42,0.08)] relative z-10 text-slate-800">
            ...
            <div class="flex flex-col items-center mb-6">
                ...
                <h3 class="text-xl font-bold text-slate-900 tracking-tight">Account Recovery</h3>
                <p class="text-xs text-slate-500 mt-1 text-center">Enter your email and we'll send you a recovery link</p>
            </div>
            ...
            <form method="POST" action="{{ route('password.email') }}" class="space-y-4">
                ...
                <!-- Email Input -->
                <div>
                    <label for="email" class="block text-xs font-medium text-slate-500 mb-1.5">Registered Email</label>
                    <input 
                        id="email" 
                        type="email" 
                        name="email" 
            ...
            <div class="mt-6 text-center text-xs">
                <a href="{{ route('login') }}" class="text-slate-500 hover:text-slate-850 font-semibold transition-colors">
